...
$versions{'zOffline-order_lib.pl'} = "20020312";

$order_lib_cc_validate = "yes";
$sc_use_secure_header_at_checkout = 'yes';
$sc_zOffline_form_prep = 0;

&add_codehook("printSubmitPage","print_zOffline_SubmitPage");
&add_codehook("set_form_required_fields","zOffline_fields");
&add_codehook("gateway_response","check_for_zOffline_response");
&add_codehook("open_for_business","check_for_zOffline_submit");
$sc_order_response_vars{"zOffline"}="process_order";
if ($sc_loading_primary_gateway =~ /yes/i) { #Hook into things ...
  &add_codehook("pre_header_navigation","zOffline_pre_header_processing");
 } 
if ($zOffline_picserve eq '') {
  $zOffline_picserve = $sc_picserve_url;
 }
if ($zOffline_picserve eq '') {
  $zOffline_picserve = 'picserve.cgi';
}
###############################################################################
sub check_for_zOffline_submit {
  if (&form_check('submit_order_form_button')) {
    if ($form_data{'gateway'} eq '') {
      $form_data{'gateway'} = $sc_gateway_name;
     }
    if ($form_data{'gateway'} eq 'zOffline') {    
      &zOffline_response_prep;
     }
   }
 }
###############################################################################
sub check_for_zOffline_response {
  if ($form_data{'process_order'}) {
    $cart_id = $form_data{'cart_id'};
    &set_sc_cart_path;
    &load_order_lib;
    &codehook("zOffline_order");
    &process_zOffline_Order;
    &call_exit;
   }
 }
###############################################################################
sub zOffline_check_and_load {
local($myname)="zOffline";
#
# if ($myname ne $sc_gateway_name) { # we are a secondary gateway, load settings
#  &require_supporting_libraries(__FILE__,__LINE__,
#     "./admin_files/$myname-user_lib.pl");
# }
if ($sc_zOffline_CC_validation ne '') {
  $sc_CC_validation = $sc_zOffline_CC_validation;
 }
$sc_stepone_order_script_url = $sc_order_script_url;
}
###############################################################################
sub zOffline_response_prep {
#
&zOffline_check_and_load;
...
###############################################################################
sub zOffline_order_form_prep {
  &zOffline_check_and_load;
  if ($sc_zOffline_form_prep == 0) {
    if (-f "$sc_verify_order_path"){
      &read_verify_file;  
     } else {
      &codehook("load_customer_info");
     }
    $sc_zOffline_form_prep = 1;
   }
  return "";
 }
###############################################################################
sub zOffline_pre_header_processing {
...
# This is now done much earlier for compatability with 3.2b through 4.0a
# &zOffline_response_prep;
...
if ($sc_paid_by_ccard =~ /yes/i) { # add these for CC entry
...
print &get_zOffline_confirm_middle(*form_data,$invoice_number,
  $customer_number,$zOffline_ver_tbldef,$messages{'ordcnf_04'},
  $zOffline_ver_tbldef2,$authPrice,$time);
...
SRC="$zOffline_picserve?secpicserve=submit_order.gif" border=0>
&nbsp;&nbsp;&nbsp;&nbsp;
</FORM>
</CENTER>
</TD><TD WIDTH="50%">
...
<INPUT TYPE=HIDDEN NAME="ofn" VALUE="zOffline"> 
<INPUT TYPE="IMAGE" NAME="Make Changes" VALUE="Make Changes" 
SRC="$zOffline_picserve?secpicserve=make_changes.gif" BORDER=0>
</FORM>
</CENTER>
</TD>
</TR>
...
###############################################################
sub get_zOffline_confirm_middle{
...
if ($sc_zOffline_show_table =~ /yes/i) {
...
$form_data{'CARDNUM'} =~ s/\ \ /\ /g; # de-over-blank it
$form_data{'CARDNUM'} =~ s/\ \ /\ /g; # de-over-blank it
$form_data{'EXPDATE'} = 
...
if ($sc_zOffline_show_table =~ /yes/i) {
  ...
  $temp = 
    &get_zOffline_confirm_middle(*form_data,$invoice_number,
    $customer_number,$zOffline_ver_tbldef,$order_ok_final_msg_tbl,
    $zOffline_ver_tbldef2,$authPrice,$time) . "<br>\n";
 } else {
