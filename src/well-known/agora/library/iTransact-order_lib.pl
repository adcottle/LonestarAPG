...
$versions{'iTransact-order_lib.pl'} = "20020312";

$sc_order_response_vars{"iTransact"}="authcode";
$sc_use_secure_header_at_checkout = 'yes';
&add_codehook("printSubmitPage","print_iTransact_SubmitPage");
&add_codehook("set_form_required_fields","iTransact_fields");
&add_codehook("gateway_response","check_for_iTransact_response");
###############################################################################
sub check_for_iTransact_response {
...
###############################################################################
sub iTransact_fields{
local($myname)="iTransact";
...
###############################################################################
sub iTransact_order_form_prep{ # load the customer info ...
  if ($sc_iTransact_form_prep == 0) {
    if (-f "$sc_verify_order_path"){
      &read_verify_file;  
     } else {
      &codehook("load_customer_info");
     }
    $sc_iTransact_form_prep = 1;
   }
  return "";
 }
###############################################################################
sub iTransact_table_setup{
#
# To use this, put this in the email_text in the manager for HTML:
#
#	<!--agorascript-pre
#	  return $iTransact_cart_table;
#	-->
#
...
local ($name,$cost);
 ...
 $iTransact_prod_in_cart = '';
 $iTransact_cart_table = '';
 $result='';
 ...
 while (<CART>)
  {
   ...
   if ($result eq '') {
     ...
     $iTransact_prod_in_cart .= "  --PRODUCT INFORMATION--\n\n";
    }
   $result .= "<TR><TD>$quantity</TD><TD>$product_id</TD>\n";
   $result .= "<TD>$name</TD><TD>$cost</TD>";
   $result .= "</TR>\n";
   $iTransact_prod_in_cart .= &cart_textinfo(*my_cart_fields);
  } # End of while (<CART>)
 close (CART);
 if ($result ne '') {
   $result .= "</TABLE></TD></TR></TABLE>\n";
  }
 $iTransact_cart_table = $result;
}

###############################################################################

sub print_iTransact_SubmitPage
...
&iTransact_table_setup;
$zemail_text = &script_and_substitute($email_text,"iTransact");
...
if ($sc_pgp_or_gpg =~ /GPG/i) { # verify with GPG
  ...
  if (($result =~ /Good Signature/i) &&
    ...
    $verification = "Could not verify iTransact signature because:\n$result\n";
   }
 }
...
$text_of_cart .= "Gateway:       iTransact\n\n";
